...
const I18N = {
  ...
  updateLangButtons() {
    document.querySelectorAll('.lang-switch button').forEach(btn => {
      const isActive = btn.dataset.lang === this._lang;
...
// === Globe Renderer ===
// Simplified continent outlines [lat_deg, lon_deg] pairs
const CONTINENTS = [
  // North America
  [[70,-168],[72,-140],[70,-100],[65,-90],[60,-65],[50,-55],[45,-65],[30,-80],[25,-80],[25,-100],[20,-105],[15,-90],[10,-85],[8,-78],[15,-60],[20,-60],[25,-80],[30,-100],[35,-120],[40,-125],[48,-125],[55,-130],[60,-140],[65,-168]],
  // South America
  [[12,-72],[12,-60],[5,-52],[0,-50],[-5,-35],[-10,-37],[-15,-40],[-23,-43],[-30,-50],[-35,-57],[-40,-65],[-45,-70],[-55,-70],[-55,-65],[-45,-60],[-35,-55],[-25,-48],[-15,-40],[-5,-50],[0,-60],[5,-65],[10,-72]],
  // Europe
  [[72,20],[70,30],[65,30],[60,30],[55,25],[50,15],[47,15],[44,12],[40,5],[36,-5],[36,0],[38,5],[40,15],[42,20],[45,25],[48,30],[50,35],[55,30],[60,25],[65,20]],
  // Africa
  [[37,10],[35,10],[30,32],[25,35],[20,40],[15,45],[10,45],[5,42],[0,42],[-5,40],[-10,40],[-15,35],[-25,30],[-30,28],[-35,20],[-35,18],[-30,17],[-25,15],[-20,12],[-15,12],[-10,14],[-5,10],[0,10],[5,5],[10,5],[15,-15],[20,-15],[25,-15],[30,-5],[35,0]],
  // Asia (simplified)
  [[72,30],[75,60],[75,90],[72,120],[70,140],[68,170],[65,175],[60,165],[55,140],[50,130],[45,135],[40,130],[35,120],[30,120],[25,105],[20,100],[15,100],[10,105],[5,105],[0,105],[-5,105],[-8,115],[-5,120],[0,115],[5,110],[10,105],[15,110],[20,110],[25,115],[30,115],[35,120],[40,115],[45,90],[50,80],[55,70],[60,60],[65,50],[70,40]],
  // Australia
  [[-12,130],[-15,125],[-20,115],[-25,115],[-30,115],[-35,118],[-38,145],[-35,150],[-30,153],[-25,150],[-20,145],[-15,140],[-12,135]],
  // India
  [[30,70],[25,70],[20,73],[15,75],[10,78],[8,77],[10,80],[15,80],[20,85],[25,80],[30,75]],
  // Middle East
  [[35,35],[30,35],[25,38],[20,40],[15,45],[20,50],[25,55],[30,50],[35,45]],
  // Southeast Asia
  [[20,100],[15,100],[10,105],[5,105],[0,105],[-5,105],[-8,115],[-5,120],[0,115],[5,110],[10,105],[15,108],[20,106]],
  // Japan
  [[45,140],[40,140],[35,135],[33,132],[35,138],[38,140],[42,145]],
  // UK
  [[58,-5],[55,-3],[52,0],[50,-5],[52,-5],[55,-5]],
  // Greenland
  [[82,-40],[78,-20],[72,-22],[68,-30],[65,-40],[70,-55],[75,-60],[80,-50]],
];
...
const GlobeRenderer = {
  ...
  init(canvasId) {
    ...
    this.resize();
    STARS = generateStars(this.canvas.width, this.canvas.height);
    this.bindEvents();
    this.animate();
  },
  ...
  project(latRad, lonRad) {
    ...
    const cosLon = Math.cos(lonRad + this.rotation);
    const sinLon = Math.sin(lonRad + this.rotation);
  ...
  drawContinent(ctx, coords, cx, cy, r, isNight) {
    ...
    const points3d = coords.map(([lat, lon]) => {
      ...
      const p = this.project(latRad, lonRad);
      return p;
    });

    const visiblePoints = points3d.filter(p => p.z > -0.15);
    ...
    if (isNight) {
      ...
      const b = Math.floor(50 + brightness * 30);
      ctx.fillStyle = `rgba(40, ${g}, ${b}, ${brightness * 0.85})`;
    }
    ctx.fill();
  ...
  drawClouds(ctx, cx, cy, r) {
    ...
    const cloudBands = [
      { lat: 0.6, width: 0.08, alpha: 0.06 },
      { lat: -0.4, width: 0.1, alpha: 0.05 },
      { lat: 0.2, width: 0.06, alpha: 0.04 },
      { lat: -0.7, width: 0.07, alpha: 0.05 },
    ];

    for (const band of cloudBands) {
      const bandY = cy - band.lat * r;
      const bandH = band.width * r;
      ctx.beginPath();
      ctx.ellipse(cx, bandY, r * 0.95, bandH, 0, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(200, 220, 255, ${band.alpha})`;
      ctx.fill();
    }

    const t = this.frameCount * 0.0003;
    for (let i = 0; i < 12; i++) {
      const angle = (i / 12) * Math.PI * 2 + t + this.rotation * 0.5;
      const lat = Math.sin(i * 2.7) * 0.6;
      const p = this.project(lat, angle);
      if (p.z < 0.1) continue;

      const cloudR = r * (0.04 + Math.sin(i * 1.3) * 0.02) * this.zoom;
      const cloudGrad = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, cloudR);
      cloudGrad.addColorStop(0, `rgba(200, 220, 255, ${0.08 * p.z})`);
      cloudGrad.addColorStop(1, 'rgba(200, 220, 255, 0)');
      ctx.beginPath();
      ctx.ellipse(p.x, p.y, cloudR, cloudR * 0.5, i * 0.5, 0, Math.PI * 2);
  ...
  drawGridLines(ctx, cx, cy, r) {
    ...
    for (let i = 0; i < 12; i++) {
      const angle = (i / 12) * Math.PI + this.rotation;
  ...
  animate() {
    ...
    this.drawOcean(ctx, cx, cy, r);
    this.drawGridLines(ctx, cx, cy, r);

    for (const cont of CONTINENTS) {
      this.drawContinent(ctx, cont, cx, cy, r, false);
    }

    this.drawSunHighlight(ctx, cx, cy, r);
    this.drawNightSide(ctx, cx, cy, r);
    this.drawClouds(ctx, cx, cy, r);
    ...
    this.drawAtmosphere(ctx, cx, cy, r);
    ...
    for (let i = this.points.length - 1; i >= 0; i--) {
      ...
      const proj = this.project(p.lat, p.lon);
      if (proj.z < 0.05) continue;

      const alpha = p.life * proj.z;
      const size = p.size * this.zoom * (1 + (1 - p.life) * 2);

      const gradient = ctx.createRadialGradient(proj.x, proj.y, 0, proj.x, proj.y, size * 4);
      gradient.addColorStop(0, `rgba(0, 230, 118, ${alpha * 0.6})`);
      gradient.addColorStop(0.3, `rgba(0, 212, 255, ${alpha * 0.3})`);
  ...
  async analyze() {
    ...
    panel.innerHTML = `
      <div class="loading">
        ...
        <p>${escapeHtml(I18N.t('analyzingEvent').replace('{event}', input))}</p>
      </div>
    `;
